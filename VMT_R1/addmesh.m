function [ newbr,taomax,ijkcube,polanum ] = addmesh( Nxyz,tao,ti,h,cr,rid,tcdeltas,deltao,dirbr,deltamin,ijkindex,arcthetas,nullpoints,polanums,xyz,btotal )
%   BVP for adding a new br while the distance of transerse adjacent mesh
%   points is larger than prescibed parameter
...
origin=[0,0,0];
dto=norm(cr(rid,:)-origin);

if dto<4
    arctheta=arcthetas(rid);
...
if norm(dirbr)<deltamin*0.01||delta==0
    newbr=r;
    taomax=1;
    return;
end
...
initao=11;
[ fn,ql,qr,qlijk,qrijk,qlstu,qrstu ] = geninit( Nxyz,r,rl,rr,initao,ijkcube,xyz ); % generate the init_points
rstu=qrstu(1,:);
...
while indexl==0%&&index>1
    ...
    [indexl,tmpbrl]=intersectfr( Nxyz,qlstu(2,:),ql(2,:),r,fn,ti,h,qlijk(2,:),delta,arctheta,dirbr,xyz,btotal);
    [flagl,accbrl] = accuracy( indexl,tmpbrl,r,delta,dirbr,deltao,arctheta);
    if flagl==1
        newbr=accbrl;
        return;
...
while indexr==0%&&index>1
    ...
    [indexr,tmpbrr]=intersectfr( Nxyz,qrstu(2,:),qr(2,:),r,fn,ti,h,qrijk(2,:),delta,arctheta,dirbr,xyz,btotal);
    [flagr,accbrr] = accuracy( indexr,tmpbrr,r,delta,dirbr,deltao,arctheta);
    if flagr==1
        newbr=accbrr;
        return;
...
dflg=[0,tao];
extao2=6;
ml=polanums(1,rid);%previous jishu in this r
mr=polanums(2,rid);
...
%         bkl=1;
        ilp=rid-ml;
        [lpplus,lpminus] = polarcheck( ilp,rid,minumflg,numcr,tao );
        if pluflg<0
            lpplus=0;
            lpminus=0;
        end
        interjishul=initao+ml*(tao-1)+lpplus-lpminus-(tao-1);
    ...
    if precomflgl==0
        [ accbr,flagl,tmpbrl,jishul,bkl,~,brsl,arctheta,mbr] = exintersect(Nxyz,dflg,bkl,exbr(2,:),r,qlstu,ql,fn,ti,h,qlijk,taol,delta,deltao,dirbr,dli,arctheta,conl,nad,brsl,xyz,btotal);
...
%     if arctheta>oarctheta&&d(2)>deltamin*0.2&&d(2)<deltam
%         newbr= exbr(2,:);
%         return;
%     end
...
%         bkr=1;
        irp=rid+mr;
        [rpplus,rpminus] = polarcheck( irp,rid,minumflg,numcr,tao );
        if pluflg<0
            rpplus=0;
            rpminus=0;
        end
        interjishur=initao+mr*(tao-1)+rpplus-rpminus-(tao-1);
    ...
    if precomflgr==0
        [ accbr,flagr,tmpbrr,jishur,bkr,~,brsr,arctheta,mbr] = exintersect(Nxyz,dflg,bkr,exbr(3,:),r,qrstu,qr,fn,ti,h,qrijk,taor,delta,deltao,dirbr,dri,arctheta,conr,nad,brsr,xyz,btotal);
...
[maxdis,~]=min(dis2);
newbr=brtaomax(dis2==maxdis,:);% convergence detected on this leaf, then br=br(tao_max);
newbr=newbr(1,:);
polanum=polanuminside(:,dis2==maxdis);
polanum=polanum(:,1);
% if norm(dirbr)<deltap
%     newbr=r;
% end
% disp('max');
% delta=maxdis;
